...
APPENDIX (many versions I tried in the process)

##the 2nd column of training dataset because that is what we need to predict about testing dataset
##also convert ordered factor to normal factor
df_target <- as.factor(df[ran,3])

##the actual values of 2nd couln of testing dataset to compaire it with values that will be predicted
 ##also convert ordered factor to normal factor
test_target <- as.factor(df[-ran,3])
#probability
prob <- attr(pr_knn, "prob")




#create prediction
pred_knn <- prediction(prob, df_test$label)

#tpr and fpr calculation
performance_knn <- performance(pred_knn, "tpr", "fpr")
plot(performance_knn)
performance_knn

tb_knn <- table(pr_knn,df_test$label)
tb_knn
##check the accuracy
accuracy_knn <- function(x){sum(diag(x)/(sum(rowSums(x)))) * 100}
accuracy_knn(tb_knn)
confusionMatrix(pr_knn, test_target)

roc(test_target, attributes(pr_knn)$prob)
knn_accuracy <- 60
knn_tpr <- 0.55
knn_fpr <- 0.33
knn_auc <- 0.611

#pradmit.number<-as.numeric(pred_tree) 
#actual.number<-as.numeric(test_target)
table(pred_tree, test_target)
pr<-prediction(pradmit.number,actual.number)
confusionMatrix(pred_tree, test_target)
performance_tree<-performance(pr,"tpr","fpr")
performance_tree
